...
import { Search, X, Filter, MapPin, Route } from "lucide-react";
...
export default function RoutesDirectoryPage() {
  ...
  const filteredRoutes = useMemo(() => {
    return routes.filter((route) => {
      ...
      const matchesFilter = !selectedFrom || route.from === selectedFrom;
      
      return matchesSearch && matchesFilter;
    });
  }, [routes, searchQuery, selectedFrom]);
  ...
  // Color gradients for different cards - only orange variations
  const cardGradients = [
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        <div className="text-center space-y-4 mb-12">
          ...
          <h1 className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-orange-600 via-orange-500 to-orange-600 bg-clip-text text-transparent">
            Browse All Routes
          </h1>
          <p className="text-gray-600 max-w-2xl mx-auto text-lg">
            Find your perfect route from our extensive collection of taxi routes across Gujarat
          </p>
        </div>
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <select
                ...
                className="flex-1 min-w-[200px] px-4 py-3 bg-white border-2 border-orange-200 rounded-xl focus:border-orange-500 focus:ring-4 focus:ring-orange-200 outline-none transition-all text-gray-900 cursor-pointer shadow-sm font-medium"
              >
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="px-5 py-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-xl font-semibold transition-all flex items-center gap-2 shadow-md hover:shadow-lg transform hover:scale-105"
                >
                  <X className="w-4 h-4" />
                  Clear Filters
                </button>
            ...
            <div className="pt-3 border-t-2 border-orange-100">
              <p className="text-sm text-gray-700 font-medium">
                Showing <span className="font-bold text-orange-600 text-base">{filteredRoutes.length}</span> of{" "}
                <span className="font-bold text-gray-900 text-base">{routes.length}</span> routes
              </p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          <div className="text-center py-16">
            ...
            <p className="mt-6 text-gray-600 font-medium text-lg">Loading routes...</p>
          </div>
        ) : filteredRoutes.length === 0 ? (
          <div className="text-center py-16 bg-gradient-to-br from-white to-orange-50 rounded-2xl shadow-xl border-2 border-orange-100">
            <div className="max-w-md mx-auto">
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-xl font-semibold transition-all shadow-md hover:shadow-lg transform hover:scale-105"
                >
                  Clear Filters
                </button>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {sortedFromKeys.map((fromKey, index) => (
              <div
                key={fromKey}
                className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-6 md:p-8 border-2 border-transparent hover:border-orange-200 relative overflow-hidden group"
              >
                ...
                <div className="space-y-3">
                  {grouped[fromKey].map((route, routeIndex) => (
                    <Link
                      key={route._id}
                      href={buildHref(route.url)}
                      className="block bg-gradient-to-r from-gray-50 to-white border-2 border-gray-200 hover:border-orange-400 hover:shadow-lg rounded-xl px-5 py-4 text-gray-800 hover:text-orange-700 transition-all duration-200 group/item relative overflow-hidden"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-base relative z-10">{route.name}</span>
                        <span className="text-orange-600 opacity-0 group-hover/item:opacity-100 transition-opacity ml-2 text-xl font-bold">→</span>
                      </div>
